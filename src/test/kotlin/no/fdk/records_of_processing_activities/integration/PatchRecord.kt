...
class PatchRecord : ApiTestContext() {
    ...
    fun `Unauthorized when not logged in`() {
        val rsp = authorizedRequest(port, "/api/organizations/111222333/records/${RECORD_TO_BE_PATCHED.recordId}", HttpMethod.PATCH,
    ...
    fun `Forbidden for wrong org access`() {
        val rsp = authorizedRequest(
            port, "/api/organizations/123/records/${RECORD_TO_BE_PATCHED.recordId}", HttpMethod.PATCH,
    ...
    fun `Forbidden for wrong read access`() {
        val rsp = authorizedRequest(
            port, "/api/organizations/111222333/records/${RECORD_TO_BE_PATCHED.recordId}", HttpMethod.PATCH,
    ...
    fun `Ok - Created - for write access`() {
        val rspBefore = authorizedRequest(
            port, "/api/organizations/111222333/records/${RECORD_TO_BE_PATCHED.recordId}", HttpMethod.GET,
